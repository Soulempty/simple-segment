...
class CrossEntropyLoss(nn.Module):
    ...
    def forward(self, score, target):
        ...
        elif self.sample == 'blob':
            return self.loss_weight*self.blob_loss(losses,target,score)
        else:
    ...
    def strap1_loss(self,losses,target,score):
        ...
        tmp_target[tmp_target == self.ignore_label] = 0
        num_hns = int(min((tmp_target>0).sum().item()*1.2,H*W*N))
        if num_hns == 0:
    ...
    def strap3_loss(self,losses,target,score):
        ...
        for i in range(N):
            ...
            num_hns = int(min((tmp_target[i]>0).sum().item()*1.2,H*W))
    ...
    def blob_loss(self,losses,target,score):
        label = target.clone()  
        label[label==self.ignore_label] = 0
        N,H,W = label.size()
        loss = self.ohem_loss(losses,target,score)
        if (label>0).sum() == 0:
            return loss
        for i in range(N):
            gt = label[i].cpu().numpy().astype(np.uint8)
            ...
            for cls_id in cls_ids[1:]:
                ...
                contours,_ = cv2.findContours(m,mode=cv2.RETR_LIST,method=cv2.CHAIN_APPROX_SIMPLE)                
                for contour in contours:
                    instance = np.zeros(m.shape,m.dtype)
                    cv2.fillPoly(instance,[contour],1) 
                    mask = label.new_tensor(instance)
                    loss += losses[i][mask==1].mean()
        return loss
...
class BlobLoss(nn.Module):
    ...
    def forward(self, score, target):
        ...
        losses = 0
        for i in range(N):
            valid = target[i].contiguous().view(-1) != self.ignore_label
            pred_mask = torch.argmax(pred_score[i],dim=0)
            gt = label[i].cpu().numpy().astype(np.uint8)
            cls_ids = np.unique(gt)
            instance_loss = 0
            for cls_id in cls_ids[1:]:
                roi = (pred_mask==cls_id).cpu().numpy() # pred cls_id
                m = np.zeros(gt.shape,gt.dtype)
                m[gt==cls_id] = 1
                contours,_ = cv2.findContours(m,mode=cv2.RETR_LIST,method=cv2.CHAIN_APPROX_SIMPLE)                
                for contour in contours:
                    ...
                    pred = pred_score[i].gather(0, label[i].unsqueeze(0)).squeeze(0)
                    loss = (-torch.log(pred)*mask).reshape(-1)[valid]
                    instance_loss +=  loss[loss!=0].mean()*class_weight[cls_id]
            losses += instance_loss
        losses /= N
